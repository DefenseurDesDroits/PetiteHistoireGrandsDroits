    ...
    */
    ...
    var getWords = function () {
      ...
      [].forEach.call(words, (wordEl) => {
        wordlist.push(wordEl.value.toUpperCase());
      });
      return wordlist;
    };
  ...
  var WordFindGame = function (puzzleEl, options) {
    ...
    */
    var select = function (target) {
      ...
      var newOrientation = calcOrientation(
        startSquare.getAttribute('y') - 0,
        startSquare.getAttribute('x') - 0,
    ...
    */
    var playTurn = function (square) {
      console.warn('PLAY TURN')
      // make sure we are still forming a valid word
      for (var i = 0, len = wordList.length; i < len; i++) {
        if (wordList[i].indexOf(curWord + square.innerText) === 0) {
          square.classList.add('selected');
          selectedSquares.push(square);
          curWord += square.innertext;
          break;
        }
      }
    };
    ...
    */
    var endTurn = function () {
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
        ...
        console.log(wordList[i] === curWord)
